...
namespace wk05_tutorial
{
    ...
    class Program
    {
        ...
        {
            PetOwner magnus = new PetOwner { FirstName = "Magnus", LastName = "Hedlund" };
            PetOwner terry = new PetOwner { FirstName = "Terry", LastName = "Adams" };
            PetOwner charlotte = new PetOwner { FirstName = "Charlotte", LastName = "Weiss" };
            PetOwner arlene = new PetOwner { FirstName = "Arlene", LastName = "Huff" };     //does not have a pet
            PetOwner narendra = new PetOwner { FirstName = "Narendra", LastName = "Pershad" };

            Pet barley = new Pet { Name = "Barley", Owner = terry };
            Pet boots = new Pet { Name = "Boots", Owner = terry };
            Pet whiskers = new Pet { Name = "Whiskers", Owner = charlotte };
            Pet bluemoon = new Pet { Name = "Blue Moon", Owner = narendra };                //owner is not in collection
            Pet daisy = new Pet { Name = "Daisy", Owner = magnus };

            // Create two lists.
            List<PetOwner> people = new List<PetOwner> { magnus, terry, charlotte, arlene };
            List<Pet> pets = new List<Pet> { barley, boots, whiskers, bluemoon, daisy };

            var CJoin =
                    from p in people
                    from pet in pets
                   select new { p.FirstName, pet.Name };
            Console.WriteLine("\n\nCross Join");
            foreach (var item in CJoin)
            {
                Console.WriteLine($"{item.FirstName + ":",-15}{item.Name}");
            ...
            //This returns all in the second set and only the matching ones in the first. 
            //It is possible that one of the pair might be missing a first partner.
            var LJoin =
                from p in people
                join pet in pets
                on p equals pet.Owner
                into a
                from b in a.DefaultIfEmpty(new Pet())
                select new { p.FirstName, b.Name };
            Console.WriteLine("\n\nLeft outer Join");
            ...
            //This returns all in the first set and only the matching ones in the second. 
            //It is possible that one of the pair might be missing a second partner.
            var RJoin =
                from pet in pets
                join p in people
                on pet.Owner equals p into joinPeoplePet
                from peep in joinPeoplePet.DefaultIfEmpty(new PetOwner())
                select new { pet.Name, peep.FirstName };
